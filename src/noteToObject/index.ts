...
import type { Accidental, AccidentalType, NoteObject, ScientificNote } from 'src/types';

const isFlat = (accidental: string) => [FLAT_LETTER, FLAT_SYMBOL].includes(accidental);
const isSharp = (accidental: string) => [SHARP_LETTER, SHARP_SYMBOL].includes(accidental);

const isSymbol = (accidental: string) => [FLAT_SYMBOL, SHARP_SYMBOL].includes(accidental);
const isLetter = (accidental: string) => [FLAT_LETTER, SHARP_LETTER].includes(accidental);
