...
const createScalesForOctave = (octave: Octave) => {
  ...
  const letterScales = symbolScales.map((scale) =>
    scale.map(accidentalToLetter),
  )
...
describe('scales', () => {
  it('should create correct major scales for all notes across 8 octaves', () => {
    ...
    const intervals = scales.map((s) => getIntervals(normalize(s)))
    ...
    expect(
      intervals.every(
        (interval) => scale.MAJOR.join(',') === interval.join(','),
      ),
    ).toBe(true)
  })
})
